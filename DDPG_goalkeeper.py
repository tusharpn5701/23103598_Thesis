...
class SoccerEnvGoalkeeper:
    ...
    def reset(self, ball_x=None, ball_y=None):
        ...
        self.state = np.array([ball_x, ball_y, self.goal_width / 2, self.goal_height / 2])  # ball_x, ball_y, goalkeeper_x, goalkeeper_y
        return self.state
    ...
    def _calculate_reward(self, gk_x, gk_y):
        ...
        # Reward for deflecting the ball
        elif distance_to_ball < 3.0:
            reward_save = 5  # Deflected the ball
        
        # Additional reward for saving shots away from the center
    ...
    def is_save(self, gk_x, gk_y):
        ball_x, ball_y = self.state[:2]
        distance_to_ball = np.sqrt((gk_x - ball_x)**2 + (gk_y - ball_y)**2)
        return distance_to_ball < 3.0  # Considered a save if within 1.5 units

# Define the Actor and Critic networks for DDPG
...
# Instantiate and train the DDPG agent for the goalkeeper
